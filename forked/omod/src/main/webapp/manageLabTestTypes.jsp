		...
		<tbody>
			<c:forEach var="tt" items="${labTestTypes}">
				<tr>
					...
					<td><span class="table-edit hvr-icon-grow"
						onclick="editTestOrder(this)"><i
							class="fa fa-eye fa-2x hvr-icon"></i></span></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
<div class="modal fade left modal-scrolling" id="sortWeightModal"
...
<script>
	function editTestOrder(ele) {
		var testTypeId = $(ele).closest("tr").find(".testTypeId").text();
		if (testTypeId != "") {
	...
	function renderSortWeight(array) {
		...
		resultsItems = resultsItems
				.concat('<table  class="table table-striped table-responsive-md btn-table table-hover mb-0" id="tb-test-type">');
		resultsItems = resultsItems.concat('<thead><tr>');
		resultsItems = resultsItems.concat('<th><a>Test Type</a></th>');
		resultsItems = resultsItems
				.concat('<th><a>Attribute Type Name</a></th>');
		...
		jQuery(array).each(
				function() {
					...
					resultsItems = resultsItems.concat('<td>' + this.testTypeId
							+ '</td>');
					resultsItems = resultsItems.concat('<td>'
							+ this.attributeTypeName + '</td>');
					resultsItems = resultsItems.concat('<td>' + this.sortWeight
							+ '</td>');
					resultsItems = resultsItems.concat('<td>'
							+ this.multisetName + '</td>');
					resultsItems = resultsItems.concat('<td>' + this.groupName
							+ '</td>');
